...
watch(() => props.direction, () => {
  ...
  parsedValue.value = parsedValue.value.replace(
    reg,
...
watch(() => props.value, (newValue, oldValue) => {
  ...
  parsedValue.value = props.value.replace(
    reg,
    '<code class="text-gray-900 dark:text-gray-200">$&</code>'
  );
}, { immediate: true });
...
<template>
  <button type="button" title="Click to copy to clipboard"
    ...
    <span class="hidden md:block font-bold text-center p-2" v-html="parsedValue">
    </span>
